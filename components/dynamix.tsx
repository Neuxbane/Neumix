...
import  {
  Avatar, Button, Checkbox, Input, Modal, Radio, Slider, Switch, Textarea, Select, Card, useModal, Spacer, Table, Calendar, DatePicker, Image, SelectItem, TableBody, TableHeader, TableRow, CardHeader, CardBody, CardFooter, ModalHeader, ModalBody, ModalFooter, TableColumn, TableCell, Tab, Tabs
...
interface modalElement { // scroll behavior always use inside
  type: 'modal',
  id: string, // modal ID which can be used to open modal via event,
  style?: React.CSSProperties,
  header: allElements[],
  content: allElements[],
  footer: allElements[],
...
export type allElements = textElement | tableElement | cardElement | containerElement | buttonElement | inputElement | imageElement | selectElement | tabsElement | textAreaElement | modalElement | switchElement | sliderElement | radioGroupElement | datePickerElement | dateRangePickerElement | calendarElement | calendarRangeElement | timeInputElement | dateInputElement | checkboxGroupElement | avatarElement | chipElement;

const DynamicUI: React.FC<{ elements: allElements[] }> = ({ elements }) => {
  // let {} = useState({});
  const renderElement = (element: allElements) => {
    switch (element.type) {
          ...
          <Card style={element.style}>
            ...
            <CardBody>
              {element.content.map((el, idx) => (
                <React.Fragment key={idx}>{renderElement(el)}</React.Fragment>
              ))}
            </CardBody>
            {element.footer && (
              <CardFooter>
      ...
      case 'modal':
        ...
        return (
          <div>
            <Button auto onClick={handleOpen}>
              Open Modal
            </Button>
            <Modal open={isVisible} style={element.style} onClose={handleClose}>
              <ModalHeader>
  ...
  return (
    <div>
      {elements.map((element, idx) => (
        <>
          {idx?<div style={{width:'1rem', height:'1rem'}}></div>:<></>}
          <React.Fragment key={idx}>{renderElement(element)}</React.Fragment>
        </>
      ))}
    </div>
  );
